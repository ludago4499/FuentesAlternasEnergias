...
def generate_demand_profile(index: pd.DatetimeIndex, peak_kw: float, industry: str, seed: int = 42) -> pd.Series:
    """Generates synthetic hourly demand (kW) for the given DatetimeIndex."""
...
<div class="demand-banner">
    <h1>🏭 Demanda Industrial e Inyección Solar</h1>
    <p>Genera un perfil típico sintético o sube tu propia curva de carga para el análisis.</p>
</div>
""", unsafe_allow_html=True)

with st.container(border=True):
    tab_gen, tab_upload = st.tabs(["⚙️ Generador de perfil típico", "📂 Subir CSV propio"])

    # ── TAB 1: Generator ──────────────────────────────────────────────────────────
    with tab_gen:
        st.markdown("<h3 style='color:#0039A6; margin-top:0;'>Perfil típico de demanda industrial</h3>", unsafe_allow_html=True)
        ...
        with col_g1:
            ...
            peak_kw = st.number_input("Demanda pico (kW)", min_value=50.0, max_value=20000.0, value=600.0, step=50.0)
        ...
        fig_prev.update_layout(template="plotly_white", height=240,
                                ...
                                yaxis_title="Demanda (kW)", margin=dict(t=10, b=30),
                                showlegend=False)
        ...
        |-----------|-------|
        | **Factor base (nocturno)** | {arch['base_factor']*100:.0f}% del pico |
        ...
        if st.button("Generar serie de tiempo", type="primary", use_container_width=True):
            if gen_end < gen_start:
                ...
                st.success(f"✅ Perfil generado: **{n_periods:,} períodos horarios** ({n_days_gen} días) | Energía total = **{df_gen['demand'].sum():,.0f} kWh**")

    # ── TAB 2: Upload ─────────────────────────────────────────────────────────────
    with tab_upload:
        st.markdown("<h3 style='color:#0039A6; margin-top:0;'>Cargar datos reales</h3>", unsafe_allow_html=True)
        try:
            ...
            st.warning("Plantilla de muestra no encontrada en el directorio /data.")
            
        st.caption("Formato requerido: columnas `timestamp` (YYYY-MM-DD HH:MM:SS) y `demand_kw` (kW, resolución horaria).")
        ...
        if uploaded:
            try:
                ...
                st.success(f"✅ Archivo cargado correctamente: {len(df_up):,} filas | Demanda Pico = {df_up['demand'].max():.1f} kW")
...
if demand_df is None:
    ...
    st.info("💡 Usando perfil de muestra automático. Ve a **Generador de perfil típico** en la parte superior para personalizar.")
...
with st.container(border=True):
    ...
    if df_irr is not None:
        ...
        st.warning("⚠️ Ejecuta primero el modelo solar (página **Análisis Solar**) para visualizar la inyección fotovoltaica real.")
    ...
    start_view = col_f1.selectbox("Ver gráfica desde la fecha:", dates, index=0)
    end_view   = col_f2.selectbox("Ver gráfica hasta la fecha:", dates, index=min(6, len(dates) - 1))
    ...
    # ── Hourly stats table & Download ─────────────────────────────────────────────
    st.write("---")
    col_t1, col_t2 = st.columns([2, 1])
    
    with col_t1:
        with st.expander("📄 Ver tabla de datos (Primeras 168 horas)"):
            disp = pd.DataFrame({
    ...
    with col_t2:
        ...
        st.download_button("📥 Descargar Serie Completa (CSV)",
